...
from noesek.vendor.hermes.cron.env_settings import cron_env_setting
from typing import Optional, Dict, List, Any, Callable, Set, Tuple, Union, Collection

logger = logging.getLogger(__name__)

from noesek.vendor.hermes.hermes_time import now as _hermes_now
from noesek.vendor.hermes.hermes_time import get_timezone
from noesek.vendor.hermes.utils import atomic_replace, atomic_write_text
...
def _job_running_in_this_process(job_id: str) -> bool:
    ...
    """
    try:
        from noesek.vendor.hermes.cron.scheduler import get_running_job_ids
...
def _secure_dir(path: Path):
    ...
    from noesek.vendor.hermes.hermes_cli.config import _secure_dir as _shared_secure_dir
    _shared_secure_dir(path)


def _secure_file(path: Path):
    """Owner-only (0600) via the shared helper (managed/container skip included)."""
    from noesek.vendor.hermes.hermes_cli.config import _secure_file as _shared_secure_file
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        from noesek.vendor.hermes.hermes_cli.config_effective import load_user_config_effective
...
def _normalize_reasoning_effort(value: Any) -> Optional[str]:
    ...
    if not text:
        return None
    from noesek.vendor.hermes.hermes_constants import parse_reasoning_effort
    ...
    *, provider: Any, model: Any, base_url: Any, no_agent: Any,
    ...
    if normalized_provider is None:
        with contextlib.suppress(Exception):
            from noesek.vendor.hermes.hermes_cli.runtime_provider import resolve_runtime_provider
...
def create_job(
    ...
    from noesek.vendor.hermes.cron.lifecycle_guard import check_gateway_lifecycle
...
def list_jobs(include_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    try:
        from noesek.vendor.hermes.cron.executions import latest_executions
...
def _claim_owner_is_dead(claim: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if parts[0] != socket.gethostname():
            return False
        from noesek.vendor.hermes.gateway.status import _pid_exists
...
def remove_job(job_id: str) -> bool:
    ...
    with _jobs_lock():
        ...
        try:
            from noesek.vendor.hermes.cron.notepad import clear_notepad
...
def mark_job_run(
    ...
    *,
    ...
    def apply(jobs, _i, job):
        ...
        from noesek.vendor.hermes.cron.unreachable_retry import clear_state, plan_retry
...
def claim_job_for_fire(
    ...
    def apply(jobs, _i, job):
        ...
        from noesek.vendor.hermes.cron.occurrences import completed_occurrence, scheduled_instant
...
def _restore_unclaimed_slot(job: Dict[str, Any], scan: _DueScan) -> Optional[str]:
    ...
    from noesek.vendor.hermes.cron.occurrences import unclaimed_pending_slot
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    if not manual_run and recurring:
        ...
        from noesek.vendor.hermes.cron.occurrences import pending_slot_stamp
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    if not value:
        return ""
    try:
        from noesek.vendor.hermes.agent.skill_utils import normalize_skill_lookup_name
