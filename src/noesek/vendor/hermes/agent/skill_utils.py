...
# Upstream path: agent/skill_utils.py
...
from noesek.vendor.hermes.hermes_constants import (
    get_config_path,
    get_skills_dir,
    get_subprocess_home,
    is_termux,
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("HERMES_KANBAN_TASK") or os.getenv("HERMES_KANBAN_BOARD"):
        try:
            from noesek.vendor.hermes.agent.delegation_context import is_dispatcher_owned_worker_context
        ...
        if owned:
            return True
    try:
        from noesek.vendor.hermes.tools.kanban_tools import _profile_has_kanban_toolset
...
def _detect_docker() -> bool:
    try:
        from noesek.vendor.hermes.hermes_constants import is_container
        return is_container()
    except Exception:
        return False
...
def _config_cache_key(config_path: Path) -> Optional[Tuple[str, int, int, int, int]]:
    ...
    try:
        from noesek.vendor.hermes.utils import file_signature
...
def display_skill_create_dir() -> str:
    ...
    from noesek.vendor.hermes.hermes_constants import display_hermes_home
...
def find_project_root(start: Optional[Path] = None) -> Optional[Path]:
    ...
    try:
        if start is None:
            from noesek.vendor.hermes.agent.runtime_cwd import scope_terminal_cwd
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        from noesek.vendor.hermes.tools.skills_guard import scan_skill_cached
        from noesek.vendor.hermes.hermes_constants import get_hermes_home
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    try:
        # See #67277.
        from noesek.vendor.hermes.tools import skills_tool as _skills_tool
