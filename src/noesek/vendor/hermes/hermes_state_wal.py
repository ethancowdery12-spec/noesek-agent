...
# Upstream path: hermes_state_wal.py
...
from noesek.vendor.hermes.hermes_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
def _wal_reset_repair_hint() -> str:
    ...
    """
    try:
        from noesek.vendor.hermes.hermes_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from noesek.vendor.hermes.hermes_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
