...
# Upstream path: tools/approval_prompt.py
...
import sys
import threading
from noesek.vendor.hermes.tools import approval_context as _ctx, approval_gateway_wait as _gw
from noesek.vendor.hermes.tools.approval_human_wait import activity_heartbeat, human_wait_window
from noesek.vendor.hermes.tools.interrupt import is_interrupted
...
def _ask_human(command: str, description: str, timeout_seconds: int, allow_permanent: bool,
    ...
    try:
        from noesek.vendor.hermes.agent.i18n import t
...
def get_plugin_manager():
    ...
    from noesek.vendor.hermes.hermes_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(*, command: str, description: str, pattern_key: str,
    ...
    try:
        from noesek.vendor.hermes.agent.redact import redact_sensitive_text
        from noesek.vendor.hermes.hermes_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def _transport_choice(attempt: dict, *, pattern_key: str, description: str):
    ...
    if attempt.get("fallback") == "builtin":
        ...
        return None, None
    from noesek.vendor.hermes.tools import approval as _a
