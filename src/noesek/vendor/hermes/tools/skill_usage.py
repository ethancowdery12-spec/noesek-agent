...
# Upstream path: tools/skill_usage.py
...
from noesek.vendor.hermes.agent.skill_utils import is_excluded_skill_path, is_external_skill_path
from noesek.vendor.hermes.utils import atomic_write_text
...
def telemetry_provenance(skill_name: str, record: Optional[Dict[str, Any]] = None) -> str:
    ...
    if ":" in skill_name:
        with suppress(Exception):
            from noesek.vendor.hermes.hermes_cli.plugins import get_plugin_manager
...
def _emit_skill_lifecycle(skill_name: str, action: str, *, record: Optional[Dict[str, Any]] = None,
    ...
    facts = record or {}
    try:
        from noesek.vendor.hermes.hermes_cli.lifecycle import has_hook, invoke_hook
...
def _relocate(src: Path, dest: Path, skill_name: str, action: str, **capture_kwargs: Any) -> Tuple[bool, str]:
    ...
    try:
        from noesek.vendor.hermes.tools import skill_ledger as _ledger
...
def _find_external_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    from noesek.vendor.hermes.agent.skill_utils import get_all_skills_dirs
