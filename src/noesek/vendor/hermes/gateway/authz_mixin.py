...
# Upstream path: gateway/authz_mixin.py
...
from noesek.vendor.hermes.gateway.bot_loop_guard import BotLoopGuard
from noesek.vendor.hermes.gateway.config import Platform
from noesek.vendor.hermes.gateway.pairing import _PLATFORM_ALLOWLIST_ENV
from noesek.vendor.hermes.gateway.session import SessionSource
from noesek.vendor.hermes.gateway.whatsapp_identity import (
...
from noesek.vendor.hermes.gateway.platforms._shared import decode_json_list_literal as _decode_json_list_literal  # noqa: E402
from noesek.vendor.hermes.gateway.platforms._shared import extra_or_secret as _extra_or_secret  # noqa: E402
from noesek.vendor.hermes.gateway.platforms._shared import platform_gate_env as _auth_env  # noqa: E402
...
def _registry_entry(platform):
    ...
    with contextlib.suppress(Exception):
        from noesek.vendor.hermes.gateway.platform_registry import platform_registry
...
class GatewayAuthorizationMixin:
    ...
    def _is_shared_bot_satellite(self, profile_name: str) -> bool:
        ...
        if not any(r.enabled and r.profile == profile_name and r.bot_profile is None for r in routes):
            return False
        from noesek.vendor.hermes.gateway.run import _multiplex_profile_homes
        try:
    ...
    def _authorization_home_for_source(self, source: SessionSource):
        ...
        if profile is None:
            from noesek.vendor.hermes.hermes_constants import get_process_hermes_home
            return get_process_hermes_home()  # the primary bot's home, never the per-turn override
        from noesek.vendor.hermes.hermes_cli.profiles import get_profile_dir
    ...
    def _legacy_telegram_chat_grant(self, source, group_user_allowlist: str) -> bool:
        ...
        from noesek.vendor.hermes.gateway.run import logger
