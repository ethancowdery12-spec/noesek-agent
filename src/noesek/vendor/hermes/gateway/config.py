...
# Upstream path: gateway/config.py
...
from noesek.vendor.hermes.hermes_cli.config import get_hermes_home
from noesek.vendor.hermes.agent.secret_scope import current_secret_scope, get_secret as _get_secret
from noesek.vendor.hermes.gateway.shutdown_watchdog import (
    DEFAULT_LOOP_WATCHDOG_INTERVAL_S,
    DEFAULT_LOOP_WATCHDOG_MAX_STRIKES,
    DEFAULT_LOOP_WATCHDOG_TIMEOUT_S,
)
from noesek.vendor.hermes.utils import is_truthy_value
...
def _has_usable_api_server_key(key: object) -> bool:
    ...
    if not key:
        return False
    try:
        from noesek.vendor.hermes.hermes_cli.auth import has_usable_secret
...
class GatewayConfig:
    ...
    def _is_platform_connected(self, platform: Platform, config: PlatformConfig) -> bool:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                ...
                from noesek.vendor.hermes.hermes_cli.plugins import discover_plugins
                discover_plugins()
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        from noesek.vendor.hermes.gateway.profile_routing import parse_profile_routes
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    from noesek.vendor.hermes.gateway.config_env import _apply_env_overrides as _impl
    _impl(config)
