...
# Upstream path: tools/registry.py
...
from noesek.vendor.hermes.hermes_constants import hermes_home_key
...
def _save_discovery_cache(cache: Dict[str, list]) -> None:
    ...
    if path is None:
        return
    try:
        from noesek.vendor.hermes.utils import atomic_json_write  # stdlib+yaml only; no cycle
        from noesek.vendor.hermes.hermes_constants import mkdir_under_hermes_home
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    try:
        ...
        pass
    try:
        from noesek.vendor.hermes.agent.secret_scope import serves_routed_profile
        from noesek.vendor.hermes.hermes_constants import get_hermes_home_override
...
def _run_check_fn_uncached(fn: Callable) -> bool:
    ...
    from noesek.vendor.hermes.agent.secret_scope import UnscopedSecretError, current_secret_scope
    try:
        return bool(fn())
...
class ToolRegistry:
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from noesek.vendor.hermes.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
