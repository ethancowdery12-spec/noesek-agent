...
# Upstream path: cron/unreachable_retry.py
...
def is_model_unreachable_failure(exc: BaseException, agent: Any = None) -> bool:
    ...
    if int(getattr(agent, "session_api_calls", 0) or 0) > 0:
        return False
    from noesek.vendor.hermes.cron.scheduler_preflight import _is_transient_provider_resolve_error
...
def plan_retry(job: Dict[str, Any]) -> bool:
    ...
    from noesek.vendor.hermes.cron.jobs import _parse_aware  # late: jobs imports this module's helpers
