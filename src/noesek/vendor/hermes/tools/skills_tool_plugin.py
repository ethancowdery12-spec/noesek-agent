...
# Upstream path: tools/skills_tool_plugin.py
...
from noesek.vendor.hermes.tools.skills_tool_setup import SkillReadinessStatus
...
def _safe_frontmatter(path: Path | None = None, *, content: str | None = None) -> Dict[str, Any]:
    ...
    from noesek.vendor.hermes.tools import skills_tool as _st
...
def _serve_skill_file(
    ...
    from noesek.vendor.hermes.tools.path_security import has_traversal_component, validate_within_dir
...
def _mark_background_review_read(path: Path) -> None:
    try:
        from noesek.vendor.hermes.tools.skill_manager_guards import mark_background_review_skill_read
...
def _preprocess_skill(content: str, skill_dir, session_id, debug_msg: str, *args) -> str:
    ...
    try:
        from noesek.vendor.hermes.agent.skill_preprocessing import preprocess_skill_content
...
def _serve_plugin_skill(
    ...
    from noesek.vendor.hermes.hermes_cli.plugins import _get_disabled_plugins, get_plugin_manager
    from noesek.vendor.hermes.tools import skills_tool as _st
...
def _plugin_skill_linked_files(skill_root: Path) -> Dict[str, List[str]] | None:
    from noesek.vendor.hermes.tools.path_security import validate_within_dir
