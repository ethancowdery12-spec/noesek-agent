...
# Upstream path: cron/executions.py
...
def _connect() -> sqlite3.Connection:
    ...
    from noesek.vendor.hermes.cron.jobs import _ensure_cron_dir
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from noesek.vendor.hermes.hermes_cli.sqlite_util import add_column_if_missing
...
def _transaction() -> Iterator[sqlite3.Connection]:
    from noesek.vendor.hermes.hermes_cli.sqlite_util import transaction
...
def _emit_execution_state(
    ...
    try:
        from noesek.vendor.hermes.agent.monitoring.cron_health import emit_execution_state
...
def _process_start_time(pid: int) -> Optional[int]:
    try:
        from noesek.vendor.hermes.gateway.status import get_process_start_time
...
def create_execution(
    ...
    from noesek.vendor.hermes.cron.occurrences import scheduled_instant as canonical_instant
...
def set_execution_occurrence(execution_id: str, instant: Optional[str]) -> None:
    ...
    from noesek.vendor.hermes.cron.occurrences import scheduled_instant
