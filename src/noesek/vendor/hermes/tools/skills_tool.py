...
# Upstream path: tools/skills_tool.py
...
from noesek.vendor.hermes.tools.registry import registry, tool_error
from noesek.vendor.hermes.hermes_cli.config import cfg_get
from noesek.vendor.hermes.agent.skill_utils import (
    EXCLUDED_SKILL_DIRS as _EXCLUDED_SKILL_DIRS, is_skill_support_path as _is_skill_support_path)
from noesek.vendor.hermes.tools.skills_tool_setup import (  # noqa: F401
...
from noesek.vendor.hermes.tools.skills_tool_plugin import (  # noqa: F401
...
from noesek.vendor.hermes.tools.skills_tool_dedup import (  # noqa: F401
...
def _skills_scan_signature(dirs_to_scan, disabled) -> tuple:
    ...
    from noesek.vendor.hermes.agent import skill_utils as _skill_utils
...
def _skill_lookup_path_error(name: str) -> Optional[str]:
    ...
    from noesek.vendor.hermes.tools.path_security import has_traversal_component
...
def load_env() -> Dict[str, str]:
    ...
    from noesek.vendor.hermes.agent.secret_scope import load_env_file
...
def _skill_utils_delegate(attr: str):
    ...
    def _delegate(*args):
        from noesek.vendor.hermes.agent import skill_utils
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    with suppress(Exception):
        from noesek.vendor.hermes.agent.skill_utils import get_external_skills_dirs
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        from noesek.vendor.hermes.hermes_cli.config import load_config
        skills_cfg = load_config().get("skills", {})
        resolved_platform = platform or os.getenv("HERMES_PLATFORM")
        if not resolved_platform:
            with suppress(Exception):
                from noesek.vendor.hermes.gateway.session_context import get_session_env
...
def _skill_search_dirs() -> Tuple[list, list, Path]:
    ...
    from noesek.vendor.hermes.agent.skill_utils import get_external_skills_dirs, get_project_skills_dirs
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    from noesek.vendor.hermes.agent.skill_utils import iter_project_skill_files, iter_skill_index_files
...
def skills_list(category: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        try:
            from noesek.vendor.hermes.hermes_cli.plugins import discover_plugins, get_plugin_manager
            discover_plugins()
...
def _resolve_plugin_skill(name, file_path, task_id, preprocess):
    ...
    from noesek.vendor.hermes.agent.skill_utils import is_valid_namespace, parse_qualified_name
    from noesek.vendor.hermes.hermes_cli.plugins import discover_plugins, get_plugin_manager
...
def _collect_skill_candidates(name, local_category_name, all_dirs):
    ...
    from noesek.vendor.hermes.agent.skill_utils import iter_skill_index_files
...
def _org_provenance_header(skill_dir: Path, active_skills_dir: Path):
    ...
    from noesek.vendor.hermes.agent.skill_utils import ORG_PROVENANCE_FILE, is_org_mirror_path, org_id_of_path
...
def _skill_readiness(frontmatter: Dict[str, Any], skill_name: str) -> Tuple[dict, dict]:
    ...
    from noesek.vendor.hermes.tools.terminal_scope import terminal_env
    ...
    if available_env_names := [e["name"] for e in required_env_vars if e["name"] not in remaining]:
        try:
            from noesek.vendor.hermes.tools.env_passthrough import register_env_passthrough
    ...
    if isinstance(required_cred_files_raw, list) and required_cred_files_raw:
        try:
            from noesek.vendor.hermes.tools.credential_files import register_credential_files
...
def _locate_skill(name: str, local_category_name: Optional[str], project_dirs: list, all_dirs):
    ...
    if skill_md is not None and project_dirs:
        from noesek.vendor.hermes.agent.skill_utils import is_quarantined_project_skill
    ...
    with suppress(Exception):
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved := parsed.get("name") or name:  # qualified forms return the canonical name
                from noesek.vendor.hermes.tools.skill_usage import bump_use, bump_view
