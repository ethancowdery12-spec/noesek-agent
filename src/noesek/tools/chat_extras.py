...
from .receipt_import import ReceiptImportInput, receipt_import_handler
from .fitness_query import FitnessQueryInput, fitness_query_handler
from .date_math import DateMathInput, date_math
from .calc import CalcInput, calc
from .code_intel import CodeIntelInput, code_intel
...
def register_chat_extras(r, conversation_id: int, controller, timeout: float) -> None:
    ...
    r.register(ToolSpec("design_system","Design-system skills: list/get schema'd themes (color, type, spacing, radius tokens) and render a self-contained styled HTML deliverable (title, subtitle, sections) into the file store. Use for landing pages, websites, slide decks, and presentations.",DesignInput,Risk.WRITE,design_system,timeout_seconds=timeout))
    ...
    r.register(ToolSpec("code_intel","Search the indexed codebase by meaning: find the function, class, or method that does X, show the relevant source for a code question, locate a symbol across languages (Python, JS/TS, Go, Rust, Java, C/C++, Ruby, C#, PHP, Bash). Actions: search, callers, callees, reindex, stats. Needs the optional codeintel extra.",CodeIntelInput,Risk.READ,code_intel,timeout_seconds=timeout))
    ...
    r.register(ToolSpec("calc","Calculate and compute arithmetic instantly - use for any math question: how much is X times Y, what is N percent of M, tips, interest, square roots, multiply, divide, powers, factorials, compound growth, unit-free numeric answers. Safe expression eval (no code execution): + - * / // % ** parentheses, sqrt log log2 log10 exp sin cos tan asin acos atan floor ceil abs round min max gcd factorial comb perm, constants pi e tau. Write percents as decimals (18% = 0.18). For date arithmetic use date_math; for running user code use code_interpreter.",CalcInput,Risk.READ,calc,timeout_seconds=timeout))
    ...
    r.register(ToolSpec("fitness_query","Answer questions about recent workouts (pace, distance, heart rate) from the chat's connected Strava account; can also log them as workout memories when asked. Informational-only - not medical advice.",FitnessQueryInput,Risk.WRITE,fitness_query_handler(conversation_id),timeout_seconds=timeout))
    r.register(ToolSpec("receipt_import","Import a receipt from the file store (photo via OCR, or pasted text) into an expense memory (merchant, date, total, currency; deduped by content hash). Informational-only money tooling - not financial advice.",ReceiptImportInput,Risk.WRITE,receipt_import_handler(conversation_id),timeout_seconds=timeout))
