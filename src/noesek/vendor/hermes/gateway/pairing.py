...
# Upstream path: gateway/pairing.py
...
from noesek.vendor.hermes.gateway.whatsapp_identity import expand_whatsapp_aliases, normalize_whatsapp_identifier
from noesek.vendor.hermes.hermes_constants import get_default_hermes_root, get_hermes_dir, get_hermes_home
from noesek.vendor.hermes.utils import atomic_json_write
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    """
    with contextlib.suppress(Exception):
        from noesek.vendor.hermes.agent.secret_scope import UnscopedSecretError, get_secret
        try:
...
def _write_allowlist_env(env_var: str, ids: list) -> None:
    ...
    with contextlib.suppress(Exception):
        from noesek.vendor.hermes.hermes_cli.config import save_env_value, remove_env_value
...
def _iter_live_gateway_adapters():
    ...
    runner = None
    with contextlib.suppress(Exception):
        from noesek.vendor.hermes.gateway.run import _gateway_runner_ref
