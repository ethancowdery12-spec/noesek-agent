...
def test_vault_add_emits_nonsecret_request(tmp_path,monkeypatch,capsys):
 monkeypatch.setenv('NOESEK_HOME',str(tmp_path));assert cli_surface.main(['vault','add','--kind','login'])==0
...
def test_password_managers_are_dry_interfaces(tmp_path,monkeypatch,capsys):
 monkeypatch.setenv('NOESEK_HOME',str(tmp_path));assert cli_surface.main(['secrets','onepassword'])==0
...
def test_profile_signed_roundtrip_and_tamper(tmp_path,monkeypatch,capsys):
 monkeypatch.setenv('NOESEK_HOME',str(tmp_path));cli_surface.main(['profile','create','a']);capsys.readouterr()
 out=tmp_path/'p.json';assert cli_surface.main(['profile','export','a','--output',str(out)])==0;capsys.readouterr()
 assert cli_surface.main(['profile','import',str(out),'--name','b'])==0;capsys.readouterr()
 d=json.loads(out.read_text());d['profile']['description']='tampered';out.write_text(json.dumps(d))
 with pytest.raises(PermissionError):cli_surface.main(['profile','import',str(out),'--name','c'])
...
def test_update_and_service_commands_only_make_signed_plans(tmp_path,monkeypatch,capsys):
 ...
 for argv in (['update','--plan'],['uninstall','--dry-run'],['gateway','install']):
  assert cli_surface.main(argv)==0;r=json.loads(capsys.readouterr().out);d=json.loads(Path(r['path']).read_text());assert not r['executed'] and a.verify_signed(d)
