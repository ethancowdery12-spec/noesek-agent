...
# Upstream path: gateway/config_loader.py
...
from noesek.vendor.hermes.gateway.config import UNAUTHORIZED_DM_BEHAVIORS, Platform, PlatformConfig, _coerce_dict, _dict_slot, _normalize_choice
...
def bridge_core_env_settings(yaml_cfg: dict, platforms_data: dict) -> None:
    ...
    from noesek.vendor.hermes.gateway.platforms._shared import profile_scoped
...
def load_yaml_layer(home: Path, gw_data: dict) -> None:
    ...
    try:
        ...
        from noesek.vendor.hermes.gateway.platform_registry import platform_registry as registry
    except Exception as e:
