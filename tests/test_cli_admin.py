...
def test_mcp_local_config_redacted(tmp_path,monkeypatch,capsys):
 ...
 assert cli_surface.main(['mcp','add','demo','--url','https://mcp.test'])==0;capsys.readouterr()
 assert cli_surface.main(['mcp','list'])==0;out=capsys.readouterr().out
 assert 'demo' in out and 'Authorization' not in out
 assert cli_surface.main(['mcp','remove','demo'])==0
...
def test_hooks_test_never_executes(tmp_path,monkeypatch,capsys):
 ...
 assert cli_surface.main(['hooks','test','pre_tool_call'])==0
...
def test_profile_local_lifecycle(tmp_path,monkeypatch,capsys):
 monkeypatch.setenv('NOESEK_HOME',str(tmp_path))
 assert cli_surface.main(['profile','create','work','--description','Work'])==0;capsys.readouterr()
 assert cli_surface.main(['profile','use','work'])==0;capsys.readouterr()
 assert cli_surface.main(['profile','list'])==0;rows=json.loads(capsys.readouterr().out);assert any(x['name']=='work' and x['active'] for x in rows)
 assert cli_surface.main(['profile','delete','work','--yes'])==0
...
def test_secret_and_vault_status_are_references_only(tmp_path,monkeypatch,capsys):
 ...
 assert cli_surface.main(['secrets'])==0;out=capsys.readouterr().out
...
def test_session_repair_is_idempotent(tmp_path,monkeypatch,capsys):
 ...
 assert cli_surface.main(['sessions','repair'])==0
