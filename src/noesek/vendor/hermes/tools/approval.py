...
# Upstream path: tools/approval.py
...
from noesek.vendor.hermes.utils import env_var_enabled, is_truthy_value
from noesek.vendor.hermes.tools import approval_context
from noesek.vendor.hermes.tools.approval_context import (
...
from noesek.vendor.hermes.tools.approval_detection import (
    _approval_key_aliases, _check_sudo_stdin_guard, detect_dangerous_command, detect_hardline_command,
)
from noesek.vendor.hermes.tools.approval_floors import (
...
from noesek.vendor.hermes.tools.approval_gateway_wait import _await_gateway_decision
from noesek.vendor.hermes.tools.approval_prompt import _present_with_selected_transport, _transport_choice, prompt_dangerous_approval
from noesek.vendor.hermes.tools.approval_smart import _smart_verdict
...
def _release_permission_mode_dependents(session_key: str) -> None:
    ...
    try:
        from noesek.vendor.hermes.tools.computer_use.tool import release_computer_use_session
...
def _permanent_set() -> set:
    ...
    """
    from noesek.vendor.hermes.hermes_constants import get_hermes_home_override, hermes_home_key
...
def _read_permanent_allowlist() -> set:
    ...
    from noesek.vendor.hermes.hermes_cli.config import load_config_readonly
...
def save_permanent_allowlist(patterns: set):
    ...
    """
    try:
        from noesek.vendor.hermes.hermes_cli.config import load_config, save_config
        config = load_config()
...
def _user_summary(outcome: str, noun: str = "command") -> str:
    from noesek.vendor.hermes.tools.approval_context import _get_approval_timeout, format_approval_window
...
def _unattended_deny(command: str, ctx: _Unattended) -> dict | None:
    ...
    if is_dangerous and not _is_permanently_approved(pattern_key):
        ...
        return result
    try:
        from noesek.vendor.hermes.tools.tirith_security import check_command_security
...
def _human_decision(spec: _GateSpec, *, command: str, description: str,
    ...
    """
    from noesek.vendor.hermes.agent.redact import redact_sensitive_text
...
def _should_skip_container_guards(env_type: str, has_host_access: bool = False) -> bool:
    ...
    try:
        from noesek.vendor.hermes.agent.terminal_env_registry import provider_flag
...
def check_all_command_guards(command: str, env_type: str,
    ...
    from noesek.vendor.hermes.agent.terminal_approval_batch import consume_prepared_guard
...
_PLUGIN_COMPAT_LAZY = {
    'DANGEROUS_PATTERNS': ('noesek.vendor.hermes.tools.approval_detection', 'DANGEROUS_PATTERNS'),
    'DANGEROUS_PATTERNS_COMPILED': ('noesek.vendor.hermes.tools.approval_detection', 'DANGEROUS_PATTERNS_COMPILED'),
    'HARDLINE_PATTERNS': ('noesek.vendor.hermes.tools.approval_detection', 'HARDLINE_PATTERNS'),
    'HARDLINE_PATTERNS_COMPILED': ('noesek.vendor.hermes.tools.approval_detection', 'HARDLINE_PATTERNS_COMPILED'),
    'HUMAN_WAIT_MARGIN_S': ('noesek.vendor.hermes.tools.approval_human_wait', 'HUMAN_WAIT_MARGIN_S'),
    'cfg_get': ('noesek.vendor.hermes.hermes_cli.config', 'cfg_get'),
    'get_plugin_manager': ('noesek.vendor.hermes.tools.approval_prompt', 'get_plugin_manager'),
    'human_wait_ceiling': ('noesek.vendor.hermes.tools.approval_human_wait', 'human_wait_ceiling'),
    'human_wait_seconds': ('noesek.vendor.hermes.tools.approval_human_wait', 'human_wait_seconds'),
    'human_wait_window': ('noesek.vendor.hermes.tools.approval_human_wait', 'human_wait_window'),
    'is_interrupted': ('noesek.vendor.hermes.tools.interrupt', 'is_interrupted'),
    'request_elicitation_consent': ('noesek.vendor.hermes.tools.approval_prompt', 'request_elicitation_consent'),
    'reset_current_observability_context': ('noesek.vendor.hermes.tools.approval_context', 'reset_current_observability_context'),
    'reset_current_session_key': ('noesek.vendor.hermes.tools.approval_context', 'reset_current_session_key'),
    'reset_hermes_interactive_context': ('noesek.vendor.hermes.tools.approval_context', 'reset_hermes_interactive_context'),
    'set_current_observability_context': ('noesek.vendor.hermes.tools.approval_context', 'set_current_observability_context'),
    'set_current_session_key': ('noesek.vendor.hermes.tools.approval_context', 'set_current_session_key'),
    'set_hermes_interactive_context': ('noesek.vendor.hermes.tools.approval_context', 'set_hermes_interactive_context'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from noesek.vendor.hermes.hermes_cli.plugin_compat import warn_once
