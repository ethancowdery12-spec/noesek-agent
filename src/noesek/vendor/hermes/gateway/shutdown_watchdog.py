...
# Upstream path: gateway/shutdown_watchdog.py
...
from noesek.vendor.hermes.gateway.restart import GATEWAY_SERVICE_RESTART_EXIT_CODE
...
def _mark_exited_quietly(exit_code: int, reason: str) -> None:
    ...
    with contextlib.suppress(Exception):
        from noesek.vendor.hermes.gateway.lifecycle_ledger import mark_exited
...
def write_loop_heartbeat(
    *, pid: Optional[int] = None, start_time: Optional[float] = None,
    ...
    with contextlib.suppress(Exception):  # after an unclean death this is the last memory record
        from noesek.vendor.hermes.gateway.lifecycle_ledger import sample_memory
...
def arm_shutdown_watchdog(
    ...
    def _watchdog() -> None:
        ...
        with contextlib.suppress(Exception):
            ...
            from noesek.vendor.hermes.gateway.status import remove_pid_file, release_gateway_runtime_lock
            remove_pid_file()
